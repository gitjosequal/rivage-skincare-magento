...
class Banner extends \Magento\Framework\Model\AbstractModel implements BannerInterface
{
    ...
    public function afterSave()
    {
        ...
        if ($image !== null) {
            $baseMediaDir = $this->filesystem->getDirectoryRead(DirectoryList::MEDIA)->getAbsolutePath();
            $baseTmpPath = $this->imageUploader->getBaseTmpPath();
            $baseTmpImagePath = $this->imageUploader->getFilePath($baseTmpPath, $image);
            $baseTmpImagePath = "$baseMediaDir$baseTmpImagePath";
            if (!$this->fileDriver->isExists($baseTmpImagePath)) { //tmp image already moved
                return $this;
            }
