...
use Amasty\Affiliate\Api\Data\ProgramExtensionInterface;
...
use Amasty\Affiliate\Model\CurrentAccount;
...
class Program extends AbstractModel implements ProgramInterface
{
    ...
    public function __construct(
        ...
        CurrentAccount $currentAccount = null // TODO move to not optional
    ) {
        ...
        $this->currentAccount = $currentAccount ?? ObjectManager::getInstance()->get(CurrentAccount::class);
    }
    ...
    {
        $affiliateAccountId = $this->currentAccount->getAccountId();
        $commissionProcessor = $this->getCommissionProcessor();

        $this->preparePriceValues(
            $commissionProcessor->getCommissionValueType($this, $affiliateAccountId),
            $commissionProcessor->getCommissionValue($this, $affiliateAccountId),
            'commission_value'
        );
        $this->preparePriceValues(
            $commissionProcessor->getCommissionTypeSecond($this, $affiliateAccountId),
            $commissionProcessor->getCommissionValueSecond($this, $affiliateAccountId),
    ...
    protected function preparePriceValues($type, $value, $valueType)
    {
        ...
        if (isset($type)) {
            if ($type == self::COMMISSION_TYPE_FIXED) {
                ...
                $this->setData($valueType, number_format($value, 2) . '%');
            }
        }
    }
    ...
    public function getAffiliateMode(): int
    {
        return (int)$this->_getData(ProgramInterface::AFFILIATE_MODE);
    }

    public function setAffiliateMode(int $affiliateMode): void
    {
        $this->setData(ProgramInterface::AFFILIATE_MODE, $affiliateMode);
    }
     ...
     * @return float
     */
    public function getComissionFromSecondOrder()
    {
        $commissionProcessor = $this->getCommissionProcessor();
        $accountId = $this->currentAccount->getAccountId();
        if (!$commissionProcessor->isDifferentCommissionFromSecondOrder($this, $accountId)) {
            return $this->getCommissionValue();
        }

        return $this->getCommissionValueSecond();
    }
    ...
    public function getExtensionAttributes(): ProgramExtensionInterface
    {
        return $this->_getExtensionAttributes();
    }

    public function setExtensionAttributes(
        ProgramExtensionInterface $extensionAttributes
    ): ProgramInterface {
        return $this->_setExtensionAttributes($extensionAttributes);
    }

    private function getCommissionProcessor(): CommissionProcessorInterface
    {
        if (!$this->commissionProcessor) {
            $this->commissionProcessor = $this->affiliateModeResolver->getCommissionProcessor($this);
        }

        return $this->commissionProcessor;
    }
}
