...
class AccountRepository extends AbstractRepository implements AccountRepositoryInterface
{
    ...
    public function save(Data\AccountInterface $entity)
    {
        if ($entity->getAccountId()) {
            $account = $this->get($entity->getAccountId());
            $account->setOrigData();
            $account->addData($entity->getData());
            if ($account->getStatus() !== $account->getOrigData(Data\AccountInterface::STATUS)) {
                $this->notificationSender->sendAffiliateStatusEmail($account, $account->getStatus());
            }
        }
