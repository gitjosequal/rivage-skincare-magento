...
class View extends \Magento\Framework\View\Element\Template
{
    ...
    public function _prepareLayout()
    {
		...
		//$branddes = $brand->getDescription();
		
		
		$this->pageConfig->getTitle()->set(__($brandname .'-'. ' Ingredients' .'-'. ' Rivage')); // browser tab title
		
		 $bdesname= "At the heart of Rivage products are our active ingredients, We draw from 58 natural ingredients; $brandname is one of them, and blend them carefully with Dead Sea salts and mud so they deliver remarkable results.";
		 
	    $this->pageConfig->setDescription(__($bdesname)); // meta description
    ...
    {
        $_objectManager = \Magento\Framework\App\ObjectManager::getInstance(); //instance 
		$storeManager = $_objectManager->get('Magento\Catalog\Block\Product\ListProduct'); 
        return $storeManager->getAddToCartPostParams($product);
    }
}
