...
class Save extends \Emizentech\ShopByBrand\Controller\Adminhtml\Items
{
	...
	//             var_dump($data);die;
				...
				try{
					...
					if($result['error']==0)
					{
						$data['mainimage'] = 'brand' . $result['file'];
					}
				} catch (\Exception $e) {
					unset($data['image']);
				}
	//             var_dump($data);die;
				if(isset($data['mainimage']['delete']) && $data['mainimage']['delete'] == '1'){
					$data['mainimage'] = '';
				}
				if(isset($data['mainimage']['value']) && strlen($data['mainimage']['value']) > 1){
					$data['mainimage'] = $data['mainimage']['value'];
				}
