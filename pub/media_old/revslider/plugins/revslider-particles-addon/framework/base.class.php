...
use \Nwdthemes\Revslider\Model\Revslider\Backwards\RevSliderGlobals;
...
class RsAddOnParticlesBase {
	...
	protected function systemsCheck() {
		...
		if(!version_compare(RevSliderGlobals::SLIDER_REVISION, RsAddOnParticlesBase::MINIMUM_VERSION, '>=')) {
	...
	protected function loadClasses() {
		...
		if($isAdmin) {
			...
			$update_admin = new RevAddOnParticlesUpdate($this->_frameworkHelper, static::$_Version);
			...
			$this->_frameworkHelper->add_filter( 'revslider_do_ajax', array($this, 'do_ajax'),10,2);
		...
		*/
		...
		new RsParticlesSliderFront($this->_frameworkHelper, static::$_Version, static::$_PluginUrl, static::$_PluginTitle, $isAdmin);
		new RsParticlesSlideFront($this->_frameworkHelper, static::$_PluginTitle);
	...
	protected function _loadPluginTextDomain(){
		...
		$this->_frameworkHelper->load_plugin_textdomain('rs_' . static::$_PluginTitle, false, static::$_PluginPath . 'languages/');
	...
	public function enqueue_admin_scripts($hook) {
		...
		if($hook === 'toplevel_page_revslider') {
			...
			if(!isset(Data::$_GET['page']) || !isset(Data::$_GET['view'])) return;
			...
			$this->_frameworkHelper->wp_enqueue_style($_handle, $_base . 'css/revslider-' . static::$_PluginTitle . '-addon-admin.css', array(), static::$_Version);
			$this->_frameworkHelper->requireScript($_handle, $_base . 'js/revslider-' . static::$_PluginTitle . '-addon-admin.js', array('jquery', 'revbuilder-admin'), static::$_Version, true);
			$this->_frameworkHelper->wp_localize_script( $_handle, 'revslider_particles_addon', self::get_var() );
	...
	private static function get_templates(){
		//load WP option
		$custom = self::$frameworkHelper->get_option('revslider_addon_particles_templates',false);
		...
		if(!isset($custom[1]["title"])){
			...
			self::$frameworkHelper->update_option('revslider_addon_particles_templates',$custom);
		}
	...
	private function save_template($template){
		...
		if($this->_frameworkHelper->update_option( 'revslider_addon_particles_templates', $custom )){
